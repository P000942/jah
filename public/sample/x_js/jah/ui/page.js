...
j$.ui.Pager=function(parent, pager , actionController){
    ...
    this.createNavigator=function(wrap){
        ...
        self_pager.add("first",(pager.Control.first===pager.Control.number)?'active':'');
        self_pager.add("back",(pager.Control.first===pager.Control.number)?'active':'');
        var pagerPosition=pager.positions();
        for (var row=pagerPosition.first; row<=pagerPosition.last; row++){
            self_pager.add(row.toString(),(row===pager.Control.number)?'disabled':'');
        }            
        self_pager.add("next",(pager.Control.last===pager.Control.number)?'active':'');
        self_pager.add("last",(pager.Control.last===pager.Control.number)?'active':'');          
    };
...
j$.ui.Page = function(){
   ...
   var modal =function(wrap,form, fixed){
       ...
       var create=function(){
          SELF.clear(); 
          var txFixed=(fixed)?'':"<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>";
          $(wrap).append("<div id='" +form.id+ "Modal' class='modal hide fade'>"
                                  +"<div id='" +form.id+ "Header' class='modal-header'>"
                                  +   txFixed
                                  +   "<h3 id='" +form.id+ "Caption' >" +form.title+ "</h3>"                                
                                  +"</div>"                                                                   
                                  +"<div id='" +form.id+ "Body' class='modal-body'>"
                                  +   "<form id='" +form.id+ "' name='" +form.id+ "'"+ j$.ui.Render.attributes(form.attributes)+ "></form></div>"
